...
| **2× blank ROM chips** | For A1200: **512KB each** (e.g. **AM27C400** or **27C400** EPROM, **120ns or 150ns**; or **flash** equivalents). **AM27C400 vs 27C400:** same type — **AM** = AMD’s prefix; **27C400** is the generic 512KB UV EPROM (other makers: **M**27C400 = ST, **MX**27C400 = Macronix). **Speed:** the number is **access time in ns** — **smaller = faster** (120ns faster than 150ns). Both 120ns and 150ns work for A1200/Alicia. With the T48 you need the adapter above for 27C400. **Alternative:** **2× 27C800** (1MB each) — often cheaper and more plentiful; use **two** chips (same as 27C400). Burn the **LO** image into the **lower 512KB** of one 27C800 (U6B), **HI** into the **lower 512KB** of the other (U6A). A1200/Alicia 42-pin sockets and board logic handle A18 (lower bank is used). **“Write the image twice”:** optionally burn the **same** 512KB image into **both halves** of each 27C800 (lower and upper 512KB), so either bank contains the correct data; the board then reads the right ROM regardless of A18. See “ROM types” and [EAB](https://eab.abime.net/showthread.php?t=112798), [TSB](https://www.tsb.space/knowledge-base/amiga-kickstart-rom-switcher-with-a-27c800-eprom/). |
| **UV eraser** (optional) | Only if you use **EPROMs** and want to **reuse** them (erase and reprogram). For a **one-time** Kickstart burn you don’t need one. EPROMs are erased with UV light. **Flash ROMs** do not need UV — you reprogram them in the programmer. |
