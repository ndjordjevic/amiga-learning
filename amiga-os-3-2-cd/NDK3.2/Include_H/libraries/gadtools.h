...
/*
**	$VER: gadtools.h 47.3 (1.1.2021)
**
**	gadtools.library definitions
**
**	Copyright (C) 2021 Hyperion Entertainment CVBA.
...
struct NewGadget
{
    ...
    const UBYTE *ng_GadgetText;		/* gadget label */
    const struct TextAttr *ng_TextAttr;	/* desired font for gadget label */
...
#define NG_GRIDLAYOUT   0x0080  /* New for V45: scale positions along 
...
struct NewMenu
{
    ...
    const UBYTE *nm_Label;	/* Menu's label */
    const UBYTE *nm_CommKey;	/* MenuItem Command Key Equiv */
...
#define GTBB_reserved1       GT_TagBase+90 /* Reserved for V50 */
#define GTMN_reserved1       GT_TagBase+91 /* Reserved for V50 */
...
#define GTBB_Scale           GT_TagBase+92 /* scale coordinates according to
					   ** the grid layout of the visual
					   ** info
					   */
#define GTBB_Headline        GT_TagBase+93 /* Include a headline in the bevel box
					   ** that is inserted on top of the 
					   ** bevel box. The argument is a 
					   ** UBYTE *
					   */
#define GTBB_HeadlinePen     GT_TagBase+94 /* Pen to be used for rendering the text
					   ** for rendering the headline.
					   */
#define GTBB_HeadlineFont    GT_TagBase+95 /* struct TextAttr to be used
					   ** for rendering the headline.
					   */
/* VisualInfo tags, new in V47 */

#define GTVI_LeftBorder      GT_TagBase+96 /* Left edge for grid layout,
					   ** default is screen->WBorLeft
					   */

#define GTVI_TopBorder       GT_TagBase+97 /* Top edge for grid layout,
					   ** default is screen->WBorTop
					   ** + sc->Font->ta_YSize + 1
					   */

/* Generic gadget placement */
#define GTVI_AlignRight      GT_TagBase+98 /* Instead of scaling the width
					   ** with the VisualInfo grid
					   ** size, ensure that the right
					   ** edge is aligned to a grid
					   ** position.
					   */
#define GTVI_AlignBottom     GT_TagBase+99 /* Instead of scaling the height
					   ** with the VisualInfo grid
...
#define GTVI_MinFontWidth    GT_TagBase+100 /* Minimum font width in pixels */

#define GTVI_MinFontHeight   GT_TagBase+101 /* Minimum font height in pixels */

/* Gadget tags, new in V47 */
#define GTMX_ScaledSpacing   GT_TagBase+102 /* Like GTMX_Spacing, but scaled
